...
| D8 | `--font-mono` が未定義トークン（`style.css:750`, `:3545` で `var(--font-mono, ...)` 参照、defensive fallback で動作） | DESIGN.md トークン体系と乖離 | Issue #105 に集約（同パターンの全スキャン） |
...
>
> **進行中 Issue**: [#105 fix(css): replace undefined CSS variable defensive-fallback pattern](https://github.com/jss826/den/issues/105) — `var(--undefined-token, fallback)` パターンの全スキャン + `--pane-bg` (Chat 系 4 箇所) 修正 + D8 (`--font-mono`) 解消を含む。
