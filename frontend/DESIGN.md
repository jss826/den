...
4. [レイアウト構造](#4-レイアウト構造)
5. [コンポーネント](#5-コンポーネント) — Tab / Pane / Modal / Keybar / Filer / Buttons / Scrollbar
...
| サイドバー expand (mobile) | `140` ~ `150` | `.sidebar-overlay`, `.filer-sidebar.sidebar-expanded` |
| グローバル UI | `160` | `#keybar` |
...
- **`.pane` の子要素 (#terminal-pane) には ID セレクタで `display: flex` を当てている**。そのため `[hidden]` が効かないので `要素[hidden] { display: none; }` を必ずセットで書く。
...
- [ ] **`var(--xxx, fallback)` の `--xxx` は定義済みか？** → defensive fallback は「typo を隠す実バグ」になりうる（過去事例: `--text-muted` `--fg-muted` `--dim` 等が未定義のまま fallback で動作し、テーマ追従しなかった / Issue #105）。`var(--xxx)` を書く前に必ず `:root` 定義を確認する。runtime に JS が設定する変数（`--keybar-layout-width` 等）のみ defensive fallback を許容
...
DESIGN.md にトークン表を新規 / 大幅追加した直後は、必ず `frontend/css/style.css` 全体に対して以下を一括スキャンする:

- `var\(--[a-z0-9-]+,\s*[^)]+\)` パターン（defensive fallback 付き参照）を Grep で列挙
- 各 token が `:root` または全テーマで定義されているか確認
- 未定義トークンは「typo の実バグ」か「JS が runtime 設定する変数」のどちらか。前者は即修正

> **理由**: DESIGN.md 制定時にこれを怠ると、規約とコードのギャップが個別 Drift として後発（D8 / D10 / D11 等）。一括で潰せば 1 PR で済む（Issue #105 の経験）。
