...
| D10 | `--font-mono` が未定義トークン（`style.css:750`, `:3545` で `var(--font-mono, ...)` 参照、defensive fallback で動作） | DESIGN.md トークン体系と乖離 | (a) `:root` に `--font-mono: ui-monospace, SFMono-Regular, ...` を定義してトークン化 / (b) 直接 fallback chain を書く。どちらかに統一 |
