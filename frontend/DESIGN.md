...
すべてのトークンは `:root` (`frontend/css/style.css:2-35`) に定義し、各テーマで上書きする。**新規トークン追加時は全 13 テーマに同時追加すること**（テーマごとに値の意味が変わるため、デフォルト継承では破綻する）。
...
| D8 | `.chat-bar-btn-danger:hover` (`style.css:1017-1020`) で `color: #f44` `background: rgba(244,68,68,0.2)` ハードコード | テーマで色が追従しない | `--error` + `--error-tint` に置換 |
| D9 | `.connection-type-badge.relay` (`style.css:2497-2500`) で `background: #f59e0b; color: #000` ハードコード | テーマ非追従、ライト/ダークで視認性変動 | `--warn` + `--bg` 等に置換、必要なら新トークン定義 |
| D10 | `--text-muted` が **未定義トークン** (`style.css:2463`, `:2503` で参照されているが `:root` / 全テーマで未定義) | fallback で親の `color` 継承 → "muted" 効果が出ていない | `--muted` の typo の可能性。`--muted` に修正 |
| D11 | `--font-mono` が未定義トークン（`style.css:750`, `:3545` で `var(--font-mono, ...)` 参照、defensive fallback で動作） | DESIGN.md トークン体系と乖離 | (a) `:root` に `--font-mono: ui-monospace, SFMono-Regular, ...` を定義してトークン化 / (b) 直接 fallback chain を書く。どちらかに統一 |
...
>
> **例外**: D10 (`--text-muted`) は実バグ（"muted" の見た目が出ていない）なので、§7 内では優先度高め。次に該当領域を触るときに即修正候補。
